...
Number_of_companies <- aggregate(Company_ID ~ Physician_ID, payments, function(x) length(unique(x)))
Number_of_companies <- Number_of_companies %>% mutate(Company_Number=Company_ID)
Number_of_companies <- Number_of_companies %>% select(-Company_ID)
...
complete_set <- complete_set %>% group_by(Company_State) %>% mutate(C_in_UT_or_OH = (Company_State == "UT"|Company_State =="OH")) %>% ungroup()
complete_set$C_in_UT_or_OH[is.na(complete_set$C_in_UT_or_OH)] <- FALSE
complete_set <- complete_set %>% group_by(Physician_ID) %>% 
  mutate(C_in_UT_or_OH=if(any(C_in_UT_or_OH == TRUE)) {TRUE} else {FALSE})
...
complete_set_physician_aggregated <- complete_set %>% select(Physician_ID, set, P_has_OwnInt, P_lives_in_MT,C_in_UT_or_OH, P_lives_in_common_OI_state, total_transactions, high_Travel_Lodg, high_Food_Bev, high_Amount_of_Cash_TX, starts_with("License_State")) %>% distinct()
...
complete_set_physician_aggregated <- categorize_license_state_frequency(complete_set_physician_aggregated, rel_freq_all %>% filter(total > 200) %>% .$License_State_1, "Amount_top_prob_Licenses") %>% mutate(Amount_top_prob_Licenses = as.factor(Amount_top_prob_Licenses))
complete_set_physician_aggregated <- categorize_license_state_frequency(complete_set_physician_aggregated, rel_freq_all %>% filter(total <= 200, total > 100) %>% .$License_State_1, "Amount_middle_prob_Licenses") %>% mutate(Amount_middle_prob_Licenses = as.factor(Amount_middle_prob_Licenses))
complete_set_physician_aggregated <- categorize_license_state_frequency(complete_set_physician_aggregated, rel_freq_all %>% filter(total <= 100) %>% .$License_State_1, "Amount_low_prob_Licenses") %>% mutate(Amount_low_prob_Licenses = as.factor(Amount_low_prob_Licenses))

#remove now unnecessary license states
complete_set_physician_aggregated <- complete_set_physician_aggregated %>% select(-starts_with("License_State"))
...
test <- inner_join(test,Number_of_companies,by="Physician_ID")
train <- inner_join(train,Number_of_companies,by="Physician_ID")
### MICHAEL END
